...
const UserStats = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const activeResponse = await axios.get('http://localhost:3000/users/active');
        const frequentResponse = await axios.get('http://localhost:3000/users/frequent');
        const inactiveResponse = await axios.get('http://localhost:3000/users/inactive');
